...
| T2 | Attacker tampers with `scope` to elevate privileges | Requested scope is validated against enabled capabilities in `MCPPluginConfig`. Unknown or over-privileged scope returns `invalid_scope`. | T4.4 |
...
| R3 | Admin denies revoking a token | All admin revocation actions in the `TokensView` component are logged. | T6.2, T8.2 |
...
| # | Threat | Mitigation | Task |
|---|--------|------------|------|
| E1 | OAuth token used to access a collection capability not granted at consent | `MCPAccessSettings` is built from the token's stored `capabilities` at validation time, not from the client's registration. Only capabilities explicitly granted by the user at consent are included. | T3.5, T4.5 |
...
| E4 | IDOR — user views or revokes another user's tokens via admin UI | The `TokensView` component enforces: admins see all tokens, users see only their own. The revoke action validates `user_id` ownership server-side (not just in the UI). | T6.2 |
...
| E8 | CSRF on consent form submission | Consent form includes a CSRF token validated server-side. Same-site cookie policy on admin session prevents cross-origin form submissions. | T4.5 |
