...
| D3 | Flood of token requests | Per-IP and per-`client_id` rate limiting on `/token` (T4.8). The combined key `ip:<ip>\|cid:<clientId>` prevents rotating client_ids from bypassing the IP limit. Note: for form-encoded `/token` requests the body is read inside the handler after the rate-limit check; the per-client_id component may not be populated for that content type, falling back to IP-only limiting. | T4.8 |
...
| Rate limiting | Per-IP combined with per-`client_id` limits on all public OAuth endpoints. The key is always `ip:<ip>\|cid:<clientId>` when a client_id is present, so rotating client_ids cannot bypass the IP-level bucket. IP is taken from `x-forwarded-for`; operators behind a reverse proxy should ensure only one trusted proxy sets this header. | T4.8 |
